...
if __name__ == "__main__":
    ...
    for i in range(1,13):
        ...
        if i == 1:
            jobcard.set_output_launch(f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime/Generated_Events_{gen}/ALP_axZ_scan_{scan}")
        ...
        if i == 1:
            for run in range(1,7):
                HEPMC_FILE = (f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime/Generated_Events_{gen}/ALP_axZ_scan_{scan}/Events/run_0{run}_decayed_1/tag_1_pythia8_events.hepmc.gz")

                with pyhepmc.open(HEPMC_FILE) as stream:
                    df, unknown = builder.build_from_events(stream)
                    
                    df.to_pickle(f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime/Generated_Events_{gen}/ALP_Z_df_Scan_{scan}_Run_{run}.pkl")
                
                DF_FILE = (f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime/Generated_Events_{gen}/ALP_Z_df_Scan_{scan}_Run_{run}.pkl")
                ...
                save_bundle(out_opt, f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime/Generated_Events_{gen}/ALP_Z_sampledfs_Scan_{scan}_Run_{run}.pkl.gz")     # Want to save the sample dataframes for use in selection pipeline
