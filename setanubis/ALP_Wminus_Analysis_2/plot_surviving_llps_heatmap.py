...
def plot_multiple_views(mass_vals, caphi_vals, heatmap_data, heatmap_weighted, output_dir, df):
    ...
    plot_heatmap(mass_vals, caphi_vals, heatmap_data,
                ...
                title='Surviving LLPs vs Mass and Coupling (pp → H → Z + ALP)',
                use_log_scale=False, use_weighted=False)
    
    # 2. Weighted counts with log scale
    plot_heatmap(mass_vals, caphi_vals, heatmap_weighted,
                os.path.join(output_dir, 'surviving_llps_heatmap_weighted_log_Z.png'),
                title='Surviving LLPs (Weighted) vs Mass and Coupling (pp → H → Z + ALP)',
                use_log_scale=True, use_weighted=True)
    
    # 3. Weighted counts with linear scale
    plot_heatmap(mass_vals, caphi_vals, heatmap_weighted,
                os.path.join(output_dir, 'surviving_llps_heatmap_weighted_linear_Z.png'),
                title='Surviving LLPs (Weighted) vs Mass and Coupling (pp → H → Z + ALP)',
                use_log_scale=True, use_weighted=True)
    ...
    plot_heatmap(mass_vals, caphi_vals, efficiency_data,
                os.path.join(output_dir, 'surviving_llps_efficiency_Z.png'),
                title='LLP Selection Efficiency vs Mass and Coupling (pp → H → Z + ALP)',
...
def main():
    ...
    parser.add_argument(
        '--csv', 
        type=str, 
        default='/usera/fs568/set-anubis/setanubis/ALP_Wminus_Analysis_2/selection_cutflow_W-.csv',
        help='Path to cutflow CSV file (default: ALP_Z_Analysis_2/selection_cutflow_data_higgs_to_alp_Z.csv)'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
