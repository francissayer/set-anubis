...
def build_selection(sel_mode: str = "standard", lifetime_s: Optional[float] = None, llp_pid: Optional[int] = None, seed: Optional[int] = None, reweight: bool = False):
    ...
    # Configure whether reweighting is enabled for this run.
    run_cfg = RunConfig(reweightLifetime=True if reweight else False, plotTrajectory=False)
    ...
    # Use sensible defaults if values not provided. Only set a reweighter
    # on the builder when reweighting has been requested.
    if reweight:
...
def extract_mass_and_coupling(bundle_path: str, scan: int, run: int) -> tuple:
    ...
    try:
        scan_dir = Path(bundle_path).parent / f"ALP_axW-_scan_{scan}"
...
Examples:
  ...
  python extract_selection_data.py --output my_selection_data.csv
    ...
    ap.add_argument(
        "--bundle-dir", 
        default="/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_W-_Runs_2",
        help="Directory containing Generated_Events_* folders (each Generated_Events_N contains bundle files). Provide the parent directory that holds the Generated_Events_* folders."
    )
    ap.add_argument(
        "--bundle-files",
        nargs="+",
    ...
    ap.add_argument(
        "--pattern",
        default="*.pkl.gz",
        help="File pattern to match (default: *.pkl.gz)"
    )
    ap.add_argument(
        "--sel-mode",
        default="standard",
    ...
    ap.add_argument(
        "--output",
        default="selection_cutflow_W-.csv",
        help="Output CSV filename (default: selection_cutflow_data_higgs_to_alp_Z.csv)"
    )
    ap.add_argument(
        "--outdir",
        default="/usera/fs568/set-anubis/setanubis/ALP_Wminus_Analysis_2",
        help="Output directory (default: /usera/fs568/set-anubis/setanubis/ALP_Z_Analysis_2)"
    )
    ...
    if os.path.exists(output_path):
        ...
        # If the CSV already contains the full filepath, prefer that for de-duplication
        if 'filepath' in existing_df.columns:
            already_processed = set(existing_df['filepath'].values)
            processed_by_filepath = True
        elif 'filename' in existing_df.columns:
    ...
    # Filter out already-processed files. If we have filepaths recorded, compare full paths
    if processed_by_filepath:
        bundle_files_to_process = [bf for bf in bundle_files if bf not in already_processed]
    else:
        bundle_files_to_process = [bf for bf in bundle_files if Path(bf).name not in already_processed]
