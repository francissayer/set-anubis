...
def extract_scan_run_from_filename(filename: str) -> tuple:
    ...
    Example: 'HNL_Condor_CCDY_qqe_sampledfs_Scan_1_Run_4.pkl.gz' -> (1, 4)
    """
...
def extract_mass_and_mixing(bundle_path: str, scan: int, run: int) -> tuple:
    """
    Extract mass and numixing (mixing angle) from bundle file and MadGraph scan_run file.
    
    Returns:
        tuple: (mass, numixing) or (None, None) if not found
    """
    import pickle
    import gzip
    ...
    # Extract numixing from MadGraph scan_run file (contains actual parameter values used)
    try:
        scan_dir = Path(bundle_path).parent / f"HNL_Condor_CCDY_qqmu_Scan_{scan}"
        if scan_dir.exists():
            ...
            if events_dir.exists():
                ...
                if scan_run_files:
                    ...
                    if len(lines) >= 2:
                        ...
                        # Find column indices
                        mass_col_idx = None
                        numixing_col_idx = None
                        
                        for i, col in enumerate(header):
                            if 'mass#9900012' in col:
                                mass_col_idx = i
                            if 'numixing#4' in col:  # numixing#4 is VmuN1 (muon mixing angle parameter)
                                numixing_col_idx = i
                        ...
                        for line in lines[1:]:
                            ...
                            if len(parts) > 0 and parts[0] == run_name:
                                ...
                                # Extract numixing
                                if numixing_col_idx is not None and numixing_col_idx < len(parts):
                                    numixing = float(parts[numixing_col_idx])
                                break
    except Exception as e:
...
def process_bundle_file(bundle_path: str, pipeline, sel_cfg, run_cfg) -> Dict:
    ...
    # Extract mass and mixing angle
    mass, numixing = extract_mass_and_mixing(bundle_path, scan, run)
...
def find_bundle_files(directory: str, pattern: str = "HNL_Condor_CCDY_qqmu_sampledfs_*.pkl.gz") -> List[str]:
...
Examples:
  ...
  python hnl_extract_selection_data.py
  
  # Specify different directory
  python hnl_extract_selection_data.py --bundle-dir /path/to/HNL_Runs_test
  
  # Process specific files
  python hnl_extract_selection_data.py --bundle-files file1.pkl.gz file2.pkl.gz
  
  # Specify output file
  python hnl_extract_selection_data.py --output my_hnl_selection_data.csv
        """
    )
    ...
    ap.add_argument(
        ...
        help="Directory containing bundle files (default: /usera/fs568/set-anubis/HNL_Runs_test)"
    )
    ap.add_argument(
        "--bundle-files",
        nargs="+",
    ...
    ap.add_argument(
        "--pattern",
        default="HNL_Condor_CCDY_qqmu_sampledfs_*.pkl.gz",
        help="File pattern to match (default: HNL_Condor_CCDY_qqmu_sampledfs_*.pkl.gz)"
    )
    ap.add_argument(
        "--sel-mode",
        default="standard",
    ...
    ap.add_argument(
        "--output",
        default="hnl_muon_selection_cutflow_data.csv",
        help="Output CSV filename (default: hnl_muon_selection_cutflow_data.csv)"
    )
    ap.add_argument(
        "--outdir",
