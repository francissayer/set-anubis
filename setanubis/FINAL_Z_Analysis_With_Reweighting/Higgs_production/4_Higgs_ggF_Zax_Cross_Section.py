...
def gamma_h_to_Za(m_h: float, m_Z: float, m_a: float, C_aphi: float, f_a: float, N_c: float, y_t: float, T_t3: float) -> float:
	"""Approximate partial width for the decay h -> Z a.

	This function implements a simplified EFT-inspired estimate of the
	partial width induced by a top-loop effective coupling. It uses a
	parametrized loop form factor (variable `F`) and the Källén function
	raised to the 3/2 power appropriate for derivative couplings.

	Args:
	- m_h: Higgs mass in GeV.
	- m_Z: Z boson mass in GeV.
	- m_a: ALP mass in GeV.
	- C_aphi: effective dimensionless coupling controlling h-Z-a.
	- f_a: ALP decay constant in GeV.
	- N_c: number of QCD colors (usually 3).
	- y_t: top-quark Yukawa coupling (dimensionless).
	- T_t3: top isospin (conventionally 1/2).
	...
	# top quark parameter integral F (from loop calculation, see e.g. arXiv:1708.00443)
	F = 0.930 + 2.64 * 10**(-6) * m_a**2

	# kinematic factor lambda^{1/2}, use lambda^{3/2} for derivative coupling
	lam_3_2 = lam**(3/2)

	# Effective coupling C_Zh_eff from top loop (see e.g. arXiv:1708.00443)
	C_Zh_eff = -(N_c * y_t**2 * T_t3 * C_aphi) * (F) / (8 * math.pi**2)
...
def higgs_cross_section(m_h: float, m_Z: float, m_a: float, C_aphi: float, f_a: float, N_c: float, y_t: float, T_t3: float, sigma_ggF: float, gamma_h_SM: float) -> Tuple[float, float]:
	...
	- C_aphi: effective coupling (dimensionless).
	- f_a: ALP decay constant in GeV.
	- N_c: number of QCD colors.
	...
	gamma_Za = gamma_h_to_Za(m_h, m_Z, m_a, C_aphi, f_a, N_c, y_t, T_t3)
...
if __name__ == "__main__":
	...
	y_t = ns['yt']
	T_t3 = 0.5  # top quark isospin (conventionally 1/2)
	...
	sigma_br, br_Za = higgs_cross_section(m_h, m_Z, m_a, C_aphi, f_a, N_c, y_t, T_t3, sigma_ggF, gamma_h_SM)
	...
	if plot_heatmap == True:
		...
		vec = np.vectorize(lambda ma, c: higgs_cross_section(m_h, m_Z, ma, c, f_a, N_c, y_t, T_t3, sigma_ggF, gamma_h_SM)[0])
		S = vec(M, C)
		...
		plt.yscale('log')
		plt.tight_layout()
		out = '/usera/fs568/set-anubis/setanubis/FINAL_Z_Analysis_With_Reweighting/Higgs_production/Plots/higgs_ggF_Za_cross_section_heatmap.png'
