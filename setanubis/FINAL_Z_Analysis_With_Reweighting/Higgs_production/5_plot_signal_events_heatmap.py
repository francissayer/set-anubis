...
def main():
    ...
    parser.add_argument('--higgs-dir', type=str,
                        default='/usera/fs568/set-anubis/setanubis/FINAL_Z_Analysis_With_Reweighting/Higgs_production',
    ...
    parser.add_argument('--cross-section-script', type=str,
                        default='/usera/fs568/set-anubis/setanubis/FINAL_Z_Analysis_With_Reweighting/Higgs_production/4_Higgs_ggF_Zax_Cross_Section.py',
                        help='Path to Higgs cross-section helper')
    parser.add_argument('--output-dir', type=str, default='/usera/fs568/set-anubis/setanubis/FINAL_Z_Analysis_With_Reweighting/Higgs_production/Plots', help='Where to save plots and CSVs')
    ...
    # discover decay channels in higgs dir
    ...
    for ch in chans:
        ...
        if not files:
            print(f'Warning: no selection_cutflow CSV found for channel {ch} in {folder}; will assume zero acceptance')
            channel_acceptances[ch] = {}
            continue
        csv_path = files[0]
        ...
        try:
            ...
            print(f'  Error loading {csv_path}: {e} — treating channel as empty')
            channel_acceptances[ch] = {}
    ...
    for (m_a, caphi) in keys:
        ...
        # compute sigma * BR(h->Z a) using higgs helper
        # Require Higgs and Z masses and axion decay constant to be present
        if 'MH' not in ns_point or 'MZ' not in ns_point:
            raise RuntimeError("Missing 'MH' or 'MZ' in UFO namespace; please provide Higgs and Z masses in the UFO parameters or via args")
        m_h = float(ns_point['MH'])
        m_Z = float(ns_point['MZ'])

        # f_a: require explicit parameter, do not fallback
        if 'fa' in ns_point:
            f_a = float(ns_point['fa'])
        elif 'FA' in ns_point:
            f_a = float(ns_point['FA'])
        else:
            raise RuntimeError("Missing required parameter 'fa' (axion decay constant) in UFO namespace")

        N_c = 3

        # top Yukawa: prefer 'yt' else compute from 'MT' if present; else error
        if 'yt' in ns_point:
            y_t = float(ns_point['yt'])
        elif 'MT' in ns_point:
            # use Standard Model relation
            vgev = 246.22056907348585
            y_t = math.sqrt(2.0) * float(ns_point['MT']) / vgev
        else:
            raise RuntimeError("Missing top Yukawa 'yt' or top mass 'MT' in UFO namespace; required to compute Higgs cross section")

        T_t3 = 0.5

        # call the higgs cross-section helper (let exceptions propagate)
        sigma_pb, br_h_za = higgs_mod.higgs_cross_section(m_h, m_Z, m_a, caphi, f_a, N_c, y_t, T_t3, args.sigma_ggf, gamma_h_sm)
    ...
    if args.save_csv:
        out_csv = os.path.join(args.output_dir, 'higgs_signal_events_data.csv')
    ...
    plot_heatmap(mass_vals, caphi_vals, heat_sig, os.path.join(args.output_dir, 'signal_events_heatmap_Higgs.png'),
                 title=f'Expected Signal Events (pp → h → Z a, L={args.luminosity} fb^-1)', use_log_scale=True,
                 colorbar_label='Expected Signal Events', show_percentage=False, sigfig=3)

    plot_heatmap(mass_vals, caphi_vals, heat_acc, os.path.join(args.output_dir, 'acceptance_heatmap_Higgs.png'),
    ...
    plot_heatmap(mass_vals, caphi_vals, heat_xsec, os.path.join(args.output_dir, 'cross_section_heatmap_Higgs.png'),
                 title='Production Cross-Section × BR(h→Z a) [pb]', use_log_scale=True,
                 colorbar_label='σ(pp→h)×BR(h→Z a) [pb]', show_percentage=False, sigfig=3)
