...
EXTRACT_SCRIPT = "/usera/fs568/set-anubis/setanubis/FINAL_Z_Analysis_With_Reweighting/pp_production/tata_Decay_Channel/2_extract_selection_data.py"
...
if bundle_dir is not None:
    ...
    if 'Generated_Events_' in path.name or 'Generated_Events_' in str(path):
        path = path.parent
    use_pattern = pattern if pattern is not None else "*.pkl.gz"
...
for coup_pos, target_coup in enumerate(target_couplings, start=1):
    ...
    jobDir = os.path.join(jobscriptDir, f"job{job_counter}_c{coup_pos}_pp_tata")
