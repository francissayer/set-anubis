...
def plot_multiple_views(mass_vals, caphi_vals, heatmap_data, output_dir, df):
    ...
    plot_heatmap(mass_vals, caphi_vals, heatmap_data,
                ...
                title='Surviving LLPs vs Mass and Coupling (pp → Z + ALP) (ALP → c c~)',
                use_log_scale=True)
    ...
    plot_heatmap(mass_vals, caphi_vals, efficiency_data,
                ...
                title='LLP Geometric & Kinematic Acceptance vs Mass and Coupling (pp → Z + ALP) (ALP → c c~)',
                use_log_scale=True)
...
def main():
    ...
    parser.add_argument(
        '--csv', 
        type=str, 
        default='/usera/fs568/set-anubis/setanubis/FINAL_Z_Analysis_With_Reweighting/pp_production/cc_Decay_Channel/selection_cutflow_cc_decay_channel.csv',
        help='Path to cutflow CSV file (default: FINAL_Z_Analysis/selection_cutflow_data_higgs_to_alp_Z.csv)'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default='/usera/fs568/set-anubis/setanubis/FINAL_Z_Analysis_With_Reweighting/pp_production/cc_Decay_Channel/Plots',
