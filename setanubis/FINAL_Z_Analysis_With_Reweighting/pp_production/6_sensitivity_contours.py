...
def main():
    default_csv = os.path.join(os.path.dirname(__file__), 'Plots', 'ppZ_signal_events_data.csv')
    parser = argparse.ArgumentParser(description='Plot sensitivity contours from ppZ signal-events CSV')
    parser.add_argument('--csv', type=str, default=default_csv, help='Path to ppZ_signal_events_data.csv')
    ...
    plot_sensitivity_contours(mass_vals, caphi_vals, heat, levels, args.output,
                              title=f'Expected Signal Events for Fermion-Coupled ALPs, $pp\\to Z a$',
