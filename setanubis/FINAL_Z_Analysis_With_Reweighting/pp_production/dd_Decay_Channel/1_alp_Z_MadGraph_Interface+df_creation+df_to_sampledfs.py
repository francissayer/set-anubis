...
if __name__ == "__main__":
    ...
    for gen in range(8,16):
        ...
        #############################################################################################
        decay_channel_index = 5 # Unique index for the dd decay channel, can be any integer (just used to differentiate the seed between different decay channels)
        process_index = 0 # Unique index for the pp > ax Z process, can be any integer (just used to differentiate the seed between different processes if needed)
        ...
        jobcard.set_output_launch(f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime_With_Reweighting/dd_Decay_Channel/Generated_Events_{gen}/ALP_axZ_scan_1")
        ...
        mlr = MadGraphLocalRunner(card_dir=f"temp_10") # Unique jobID to create a unique directory in MadGraph to store the temporary cards for this run and avoid conflicts with other runs. Can be any integer, just make sure it's different from the one used for other decay channels and processes to avoid conflicts in the MadGraph cache.
        ...
        for run in range(1,10):
            HEPMC_FILE = (f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime_With_Reweighting/dd_Decay_Channel/Generated_Events_{gen}/ALP_axZ_scan_1/Events/run_0{run}_decayed_1/tag_1_pythia8_events.hepmc.gz")

            with pyhepmc.open(HEPMC_FILE) as stream:
                df, unknown = builder.build_from_events(stream)
                
                df.to_pickle(f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime_With_Reweighting/dd_Decay_Channel/Generated_Events_{gen}/ALP_Z_df_Scan_1_Run_{run}.pkl")
            
            DF_FILE = (f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime_With_Reweighting/dd_Decay_Channel/Generated_Events_{gen}/ALP_Z_df_Scan_1_Run_{run}.pkl")
            ...
            save_bundle(out_opt, f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime_With_Reweighting/dd_Decay_Channel/Generated_Events_{gen}/ALP_Z_sampledfs_Scan_1_Run_{run}.pkl.gz")     # Want to save the sample dataframes for use in selection pipeline
            
            print(out_opt["LLPs"])
            
        for run in range(10,13):
            HEPMC_FILE = (f"/raid/anubis/sensitivityStudyData/ALPs/fermionCoupled/ALP_Z_FINAL_Default_Lifetime_With_Reweighting/dd_Decay_Channel/Generated_Events_{gen}/ALP_axZ_scan_1/Events/run_{run}_decayed_1/tag_1_pythia8_events.hepmc.gz")
