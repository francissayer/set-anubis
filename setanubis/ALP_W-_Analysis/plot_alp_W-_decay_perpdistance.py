...
def plot_individual_distributions(data_dict_list, output_dir, caphi, use_log=True):
    ...
    plt.suptitle(fr'ALP $r_\perp$ Distributions (W- associated production, Perp. Distance from Beamline, $C_{{a\phi}}$ = {caphi_str})', 
                 fontsize=14, fontweight='bold', y=0.995)
    plt.tight_layout()
    
    output_path = os.path.join(output_dir, 'alp_rperp_Wminus_individual.pdf')
...
def plot_overlay_distribution(data_dict_list, output_dir, caphi, use_log=True):
    ...
    output_path = os.path.join(output_dir, 'alp_rperp_Wminus_overlay.pdf')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_statistics_vs_mass(data_dict_list, output_dir, caphi):
    ...
    output_path = os.path.join(output_dir, 'alp_rperp_Wminus_vs_mass.pdf')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
