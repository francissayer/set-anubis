...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='/usera/fs568/set-anubis/setanubis/FINAL_Fermion+Higgs_Coupling_Analysis_With_Reweighting/Higgs_production/Plots', help='Where to save plots and CSVs')
    ...
    parser.add_argument('--czh-eff', type=float, default=0.015,
    ...
    for (m_a, caphi) in keys:
        ...
        # call the higgs cross-section helper using the helper's signature:
        # higgs_cross_section(m_h, m_Z, m_a, f_a, C_Zh_eff, sigma_ggF, gamma_h_SM)
        sigma_pb, br_h_za = higgs_mod.higgs_cross_section(m_h, m_Z, m_a, f_a, args.czh_eff, args.sigma_ggf, gamma_h_sm)
    ...
    plot_heatmap(mass_vals, caphi_vals, heat_sig,
                 os.path.join(args.output_dir, 'signal_events_heatmap_Higgs.png'),
                 title=(f'Expected Signal Events (pp → h → Z a, L={args.luminosity} fb^-1, '
                        f'$C_{{Zh}}^{{eff}}={args.czh_eff:g}$)'),
                 use_log_scale=True,
                 colorbar_label='Expected Signal Events',
                 show_percentage=False,
                 sigfig=3)

    plot_heatmap(mass_vals, caphi_vals, heat_acc,
                 os.path.join(args.output_dir, 'acceptance_heatmap_Higgs.png'),
                 title='Geometric & Kinematic Acceptance (weighted by ALP BR)',
                 use_log_scale=True,
                 colorbar_label='Acceptance ε',
                 show_percentage=args.acceptance_percent,
                 sigfig=3)

    plot_heatmap(mass_vals, caphi_vals, heat_xsec,
                 os.path.join(args.output_dir, 'cross_section_heatmap_Higgs.png'),
                 title=(f'Production Cross-Section × BR(h→Z a) [pb], $C_{{Zh}}^{{eff}}={args.czh_eff:g}$'),
                 use_log_scale=True,
                 colorbar_label='σ(pp→h)×BR(h→Z a) [pb]',
                 show_percentage=False,
                 sigfig=3)
