"""
Compare MATHUSLA200 converted-data contours with simulated MATHUSLA200 contours.
...
MATHUSLA200 grid files. The converted experimental polylines (files named
`MATHUSLA200_BR_*.csv` or `MATHUSLA200.csv`) are discovered automatically by the
plotting module and overlaid as limits.

Usage:
	python 10_compare_MATHUSLA200_converted_data_and_simulated_data.py \
		--simulated "Plots/Simulated_MATHUSLA200_higgs_signal_events_data_mumu_BR_*.csv" \
		--output Plots/compare_MATHUSLA200_converted_vs_simulated.png
"""
...
def main():
	parser = argparse.ArgumentParser(description='Overlay Simulated MATHUSLA200 contours over converted-data polylines')
	default_sim = os.path.join(os.path.dirname(__file__), 'Plots', 'Simulated_MATHUSLA200_higgs_signal_events_data.csv')
	parser.add_argument('--simulated', type=str, default=default_sim,
						help='Path, glob, or folder for simulated MATHUSLA200 CSV(s)')
	default_out = os.path.join(os.path.dirname(__file__), 'Plots', 'compare_MATHUSLA200_converted_vs_simulated.png')
	...
	if '*' in sim_arg or '?' in sim_arg:
		...
		sim_list = sorted(glob.glob(os.path.join(sim_arg, 'Simulated_MATHUSLA200_higgs_signal_events_data_mumu_BR_*.csv')))
	else:
		...
		if os.path.abspath(sim_arg) == os.path.abspath(default_sim):
			...
			if pb_files:
				...
				# fallback to any matching Simulated_MATHUSLA200 files
				sim_list = sorted(glob.glob(os.path.join(os.path.dirname(__file__), 'Plots', 'Simulated_MATHUSLA200*')))
		else:
	...
	if not sim_list:
		raise FileNotFoundError(f'No simulated MATHUSLA200 CSV files found for pattern: {args.simulated}')
	...
	title = 'MATHUSLA200: Own simulated contours over MATHUSLA data limits'
	...
	# Draw simulated MATHUSLA200 contours on top of experimental polylines
	...
	if sim_any:
		try:
			sim_group_handle = Line2D([0], [0], color=sim_color, lw=2.8, linestyle='-', label='MATHUSLA200 (simulated)')
	...
	try:
		...
		ax.set_xlabel(r'Effective Higgs Coupling $C_{Zh}^\text{eff}$', fontsize=20)
