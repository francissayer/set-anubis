...
EXTRACT_SCRIPT = "/usera/fs568/set-anubis/setanubis/FINAL_Compare_To_Mathusla_Fermion+Higgs_Coupling_Analysis_With_Reweighting/Higgs_production/mumu_Decay_Channel/4_extract_selection_data_for_MATHUSLA40.py"
...
if default_outdir and default_output:
    # Look for detector-suffixed CSVs first (e.g. selection_cutflow_..._MATHUSLA40.csv)
...
if args.rebuild_missing:
    ...
    if len(missing_set) == 0:
        ...
        for (t_norm, b_norm), items in sorted(missing_by_combo.items()):
            ...
            job_counter += 1
            jobDir = os.path.join(jobscriptDir, f"job{job_counter}_c{coup_pos}_b{br_pos}_M40")
