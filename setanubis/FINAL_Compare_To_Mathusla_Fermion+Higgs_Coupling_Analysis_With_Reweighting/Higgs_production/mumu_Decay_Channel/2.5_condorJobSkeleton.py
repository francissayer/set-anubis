...
EXTRACT_SCRIPT = "/usera/fs568/set-anubis/setanubis/FINAL_Compare_To_Mathusla_Fermion+Higgs_Coupling_Analysis_With_Reweighting/Higgs_production/mumu_Decay_Channel/2_extract_selection_data.py"
...
# Load default coupling and BR lists and other defaults from the extract script
...
# Read existing output CSV to determine already-processed (coupling,BR,bundle) entries
...
if output_csv and os.path.exists(output_csv):
    try:
        ...
        print(f"Found existing CSV with {len(existing_df)} entries; will skip already-processed bundles for matching coupling/BR")
    except Exception as e:
        print(f"Warning: failed to read existing output CSV {output_csv}: {e}")
else:
    print("No existing output CSV found; will create jobs for all coupling/BR combinations")
...
sys.argv = ["{EXTRACT_SCRIPT}", "--force-coup-pos", "{coup_pos}", "--BR_mu", "{br_mu}"]
# Call main
if hasattr(mod, 'main'):
    mod.main()
else:
