...
DEFAULT_OUTPUT = "selection_cutflow_mumu_decay_channel_MATHUSLA40.csv"
...
DEFAULT_DETECTOR = "MATHUSLA40"
# MATHUSLA40 geometry defaults (meters) — canonical numbers from the MATHUSLA CDR
# Updated to match the configuration in arXiv:2504.01999 (Table 1 and Section 2)
# The paper states:
#  - Detector volume ~40m x 45m x 16m
#  - Decay volume ~40m x 40m x 11m
#  - Distance from CMS IP: 82-93m vertical, 70-110m horizontal along beam axis
DEFAULT_MATHUSLA_FOOTPRINT_X = 40.0 # From decay volume footprint
DEFAULT_MATHUSLA_FOOTPRINT_Z = 40.0 # From decay volume footprint
# Floor height (concrete floor level) above the CMS IP used in the CDR GEANT implementation.
# The vertical distance from IP is 82m to 93m, so the floor is at 82m.
DEFAULT_MATHUSLA_FLOOR_HEIGHT_ABOVE_IP = 82.0
# Vertical offset (m) of the first ceiling tracking layer above the floor.
# Since decay volume is 11m, the first tracking layer starts just above it.
DEFAULT_MATHUSLA_FIRST_LAYER_OFFSET = 11.0
# Number of ceiling tracking layers and spacing
# The paper states "6 in ceiling (top 4m, 0.8m apart)"
DEFAULT_MATHUSLA_LAYER_COUNT = 6
DEFAULT_MATHUSLA_LAYER_SPACING = 0.8
# Approximate decay-volume height from floor to ceiling tracking modules (m)
DEFAULT_MATHUSLA_DECAY_VOLUME_HEIGHT = 11.0

# Default horizontal (Z) offset of the decay-volume centre (m) relative to the IP.
# Table 1 of arXiv:2504.01999 quotes a horizontal distance range of 70--110 m;
# use 90 m as the nominal central value but expose as a configurable constant.
DEFAULT_MATHUSLA_CENTER_Z_OFFSET = 90.0


class MATHUSLA40Geometry:
    """A simple, self-contained approximation of the MATHUSLA-40 geometry.
    ...
    layers placed at constant Y (vertical) positions above the IP and
    multiple vertical RPC layers at constant Z (horizontal) positions.
    """
    ...
    def __init__(
        ...
        # relative to the CMS IP. Table 1 of arXiv:2504.01999 quotes 70--110 m;
