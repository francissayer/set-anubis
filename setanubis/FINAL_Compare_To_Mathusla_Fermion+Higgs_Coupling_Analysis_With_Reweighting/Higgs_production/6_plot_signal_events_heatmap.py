...
def main():
    ...
    def compute_mu_heatmaps():
        ...
        for br in br_vals:
            ...
            out_csv = os.path.join(args.output_dir, f'higgs_signal_events_data_mumu_BR_{str(br).replace(".","p")}.csv')
            ...
            out_png = os.path.join(args.output_dir, f'signal_events_heatmap_mumu_BR_{str(br).replace(".","p")}.png')
            plot_heatmap(mass_vals_grid, caphi_vals_grid, heat_sig, out_png,
                         title=(f'Expected Signal Events (mumu), BR_mu={br}, mass={args.mass} GeV'),
