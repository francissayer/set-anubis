"""
Plot sensitivity contours from an extended Higgs signal-events CSV for MATHUSLA40.

Reads `Plots/Simulated_MATHUSLA40_higgs_signal_events_data.csv` (produced by
`7.5_plot_signal_events_heatmap_for_MATHUSLA40.py`) and draws a heatmap of expected signal
events with contour lines at specified event counts. One contour is
highlighted at 4 events by default.

Usage:
    python 8.5_sensitivity_contours_for_MATHUSLA40.py --csv /path/to/Simulated_MATHUSLA40_higgs_signal_events_data.csv
...
def plot_sensitivity_contours_overlay(csv_paths, levels, output_path,
    ...
    for idx, d in enumerate(datasets):
        ...
        # group display (prefer explicit MATHUSLA40 simulated label when present)
        try:
...
def main():
    default_csv = os.path.join(os.path.dirname(__file__), 'Plots', 'Simulated_MATHUSLA40_higgs_signal_events_data.csv')
    parser = argparse.ArgumentParser(description='Plot sensitivity contours from Higgs signal-events CSV')
    parser.add_argument('--csv', type=str, default=default_csv, help='Path to Simulated_MATHUSLA40_higgs_signal_events_data.csv')
    parser.add_argument('--output', type=str, default=os.path.join(os.path.dirname(__file__), 'Plots', 'Simulated_MATHUSLA40_sensitivity_contours.png'), help='Output PNG path')
    ...
    if '*' in csv_path or '?' in csv_path:
        ...
        csv_list = sorted(glob.glob(os.path.join(csv_path, 'Simulated_MATHUSLA40_higgs_signal_events_data_mumu_BR_*.csv')))
    else:
    ...
    if len(csv_list) > 1:
        ...
        title_base = 'Simulated MATHUSLA40 Expected Signal Events for Fermion-Coupled ALPs, $pp\\to H \\to Z a$'
