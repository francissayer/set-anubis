...
def plot_individual_distributions(data_dict_list, output_dir, caphi, use_log=True):
    ...
    plt.suptitle(fr'ALP Distance from IP Distributions (W+ associated production, $C_{{a\phi}}$ = {caphi_str})', 
                 fontsize=14, fontweight='bold', y=0.995)
    plt.tight_layout()
    
    output_path = os.path.join(output_dir, 'alp_distance_ip_Wplus_individual.pdf')
...
def plot_overlay_distribution(data_dict_list, output_dir, caphi, use_log=True):
    ...
    caphi_str = f"{caphi:.6f}" if caphi < 0.01 else f"{caphi:.3f}"
    ax.set_title(fr'ALP Distance from IP Distributions: All Masses (W+ associated production, $C_{{a\phi}}$ = {caphi_str})', 
    ...
    output_path = os.path.join(output_dir, 'alp_distance_ip_Wplus_overlay.pdf')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_statistics_vs_mass(data_dict_list, output_dir, caphi):
    ...
    output_path = os.path.join(output_dir, 'alp_distance_ip_Wplus_vs_mass.pdf')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
